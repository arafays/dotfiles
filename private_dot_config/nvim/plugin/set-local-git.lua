...
local function save_identities(identities)
	...
	for _, id in ipairs(identities) do
		file:write(
			string.format("  { name = %q, email = %q, signingkey = %q },\n", id.name, id.email, id.signingkey or "")
		)
	end
	file:write("}\n")
	file:close()
...
local function ensure_git_config()
	...
	if vim.g.git_identity_shown then
		return
	end
	vim.g.git_identity_shown = true
	...
	local function create_new_identity(identity)
		...
		vim.ui.input({ prompt = "Enter full name: " }, function(name)
			...
			vim.ui.input({ prompt = "Enter email: " }, function(email)
				...
				vim.ui.input(
					{ prompt = "Enter GPG signing key (optional press enter to leave blank): " },
					function(signing_key)
						...
						vim.notify(
							"Local Git user set to: " .. identity.name .. " <" .. identity.email .. ">",
							vim.log.levels.INFO
						)
					end
				)
	...
	-- Use vim.ui.select (snacks picker may have compatibility issues with Neovim 0.11)
...
-- Wait for UIEnter event and schedule the identity check
vim.api.nvim_create_autocmd("UIEnter", {
	...
	desc = "Set local git identity after UI is ready",
	once = true,
})
