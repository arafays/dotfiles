...
description: Use when writing Upwork proposals for Abdul Rafay Shaikh; matches each job post to his portfolio, skills, experience, and screening-question answers; grounds in live Upwork data when the upwork MCP server is enabled
...
This skill writes the letter; it needs no MCP when the job posting is pasted as text. The `upwork` MCP server is **disabled by default** in `~/.config/opencode/opencode.jsonc` (`mcp.servers.upwork`) and is required when:

- the target is a job URL or job id (must be fetched), or
- the letter should be grounded in Abdul's live Upwork history.

If upwork tools are unavailable in those cases, **ask the user to enable the server and wait — never edit any config file yourself**. Resume once they confirm. If they prefer not to enable it, ask them to paste the job posting text instead.

## Upwork MCP usage

Do not assume tool names, actions, or parameter names — the upwork MCP evolves. Discover the current tools by searching the tool catalog for the upwork server namespace, read each tool's live description/signature before calling it, and follow any workflow rules the tools embed. When a tool's own description conflicts with this file, the tool wins. This section references *capabilities* (job details, profile, past proposals, highlights), not identifiers.

When the MCP is available, ground the proposal in **real data** before writing:

- Full job details: description, screening questions, budget, preferred qualifications, client hiring record — a job URL, numeric id, or `~02…` ciphertext is usually accepted as-is.
- Abdul's live profile: title, overview, skills, job aggregates — never contradict it.
- Recent past proposals, especially offered/hired ones — the strongest tone and structure templates.
- Portfolio projects and certificates attachable to a proposal as highlights.

Never invent what live data can provide; when live data conflicts with the static portfolio below, the live data wins.
...
3. **If no clear job posting is found** in the user's message, stop immediately and ask: "Please paste the Upwork job URL or the full job description text." If the reply is a job URL or job id, fetch the full posting via the upwork MCP (see the Gate) instead of asking again.
...
This skill produces the letter and screening answers only. When the user wants the proposal **submitted** through Upwork, load the `upwork-proposal-submitter` skill and follow its draft-preview-confirm flow — never submit directly. It handles bid positioning, connects cost, boosts, attachments, invitations, interviews, offers, and milestones.
